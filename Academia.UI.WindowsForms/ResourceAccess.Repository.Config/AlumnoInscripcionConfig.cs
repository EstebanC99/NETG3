...
namespace ResourceAccess.Repository.Config
{
    public class AlumnoInscripcionConfig : EntityTypeConfiguration<AlumnoInscripcion>
    {
        public AlumnoInscripcionConfig()
        {
            ...
            this.HasRequired(m => m.Curso).WithMany().Map(m => m.MapKey("ID_Curso")).WillCascadeOnDelete();
            this.HasRequired(m => m.Alumno).WithMany().Map(m => m.MapKey("ID_Persona_Alumno")).WillCascadeOnDelete();
